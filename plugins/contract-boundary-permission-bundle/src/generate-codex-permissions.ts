...
# Do not edit by hand; regenerate from the dependency graph.
#
# Security boundary:
...
Generates Codex permission profiles from JS/TS boundary dependency graph JSON.
  ...
  for (const boundary of graph.boundaries ?? []) {
    ...
    if (!isSameOrChildPath(boundary.root, boundary.readme)) {
      diagnostics.push({
        ...
        message: "Boundary README must be inside its boundary root.",
        path: boundary.readme,
      });
    }
...
function buildProfiles(graph: DependencyGraph): Profile[] {
  ...
  for (const edge of graph.boundaryEdges ?? []) {
    ...
    const deps = depsByBoundary.get(edge.from);
    if (deps) {
      deps.add(edge.to);
    }
  }
...
function renderConfigToml(profiles: Profile[], options: { graphPath: string; defaultProfile: string | null }): string {
  const lines: string[] = [GENERATED_HEADER.trimEnd(), "", `# Source graph: ${options.graphPath}`];
